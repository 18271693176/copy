...
class Scheduler:
    ...
    def load_past_jobs(self):
        ...
        for dir_name in sorted(os.listdir(os.path.join(config_value('jobs_dir'),'alljobs'))):
            if os.path.isdir(os.path.join(config_value('jobs_dir'), 'alljobs/'+dir_name)):
                # Make sure it hasn't already been loaded
                if dir_name in self.jobs:
                    continue

                try:
                    job = Job.load(job_id=dir_name)
