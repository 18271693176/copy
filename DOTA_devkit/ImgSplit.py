...
class splitbase():
    def __init__(self,
        ...
        """
        :param basepath: base path for dota data 图片所在文件夹地址
        :param outpath: output base path for dota data, 分割图片输出文件夹
        the basepath and outputpath have the similar subdirectory, 'images' and 'labelTxt' 生成的图片和label 目录结构一样
        :param code: encodeing format of txt file 解决txt文件编码问题
        :param gap: overlap between two patches  两个patch之间的重叠
        :param subsize: subsize of patch 小尺寸的块
        :param thresh: the thresh determine whether to keep the instance if the instance is cut down in the process of split 在分割过程中，如果实例被分割，则由thresh决定是否保留该实例
        :param choosebestpoint: used to choose the first point for the  用于选择第一点
        :param ext: ext for the image format  图片格式后缀
        """
    ...
    def SplitSingle(self, name, rate, extent):
        ...
        if np.shape(img) == ():
            return
        fullname = os.path.join(self.labelpath, name + '.txt') # 分割标签
    ...
    def splitdata(self, rate):
        """
        :param rate: resize rate before cut  在分割之前调整比率
        """
        imagelist = GetFileFromThisRootDir(self.imagepath) # 是否要根据后缀过滤文件 不传参就是不过滤 图片文件列表
        imagenames = [util.custombasename(x) for x in imagelist if (util.custombasename(x) != 'Thumbs')]
        for name in imagenames: # 遍历分割图片
            self.SplitSingle(name, rate, self.ext)

if __name__ == '__main__':
    # example usage of ImgSplit  传入图片 分割完之后会放在一个文件夹中
